...
class GuessingGame<E>(
    ...
    private fun evaluateGuess(guess: Option<Int>, num: Int, name: String?): Kind<E, Kind<ForOption, Unit>> =
        guess
    ...
    private fun readGuess(): Kind<E, Option<Int>> =
        monad.run {
            console.readLn().map { input -> Try { input!!.toInt() }.toOption() }
        }

    private fun checkContinue(ifNo: () -> Kind<E, Unit>, ifYes: () -> Kind<E, Unit>): Kind<E, Unit> =
        monad.run {
